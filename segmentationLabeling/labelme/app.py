...
from .shape import DEFAULT_FILL_COLOR
from .shape import DEFAULT_LINE_COLOR
from .shape import Shape
from .widgets import Canvas
from .widgets import ColorDialog
from .widgets import EscapableQListWidget
from .widgets import LabelDialog
from .widgets import LabelQListWidget
from .widgets import ToolBar
from .widgets import ZoomWidget
from .widgets import label_file_for_seg
from .widgets import changeAlien
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        quit = action('&Quit', self.close, shortcuts['quit'], 'quit',
                      'Quit application')
        open_ = action('&Open File', self.openFile, shortcuts['open'], 'open',
        ...
        toCoco = action('toCoco', self.jsonToCoco, 'C',
                        'toCoco', 'json to coco',
                        checkable=False,
                        enabled=False)
        ...
        delete = action('Delete\nPolygons', self.deleteSelectedShape,
                        shortcuts['delete_polygon'], 'cancel',
                        'Delete the selected polygons', enabled=False)
        copy = action('Duplicate\nPolygons', self.copySelectedShape,
        ...
        self.actions = utils.struct(
            ...
            save=save, saveAs=saveAs, open=open_, close=close, toCoco=toCoco,
    ...
    def openDirDialog(self, _value=False, dirpath=None):
        ...
        if targetDirPath is not None and len(targetDirPath) > 1:
            self.createJsonFolder(targetDirPath)
            self.importDirImages(targetDirPath)
            self.actions.toCoco.setEnabled(True)

    def createJsonFolder(self, targetDirPath):
        try:
            jsonpath = os.path.join(targetDirPath, 'JSON Files')
            if not (os.path.isdir(jsonpath)):
                os.makedirs(jsonpath)
            self.output_dir = jsonpath
        except OSError as e:
            if e.errno != errno.EEXIST:
                print("Failed to create directory!")
                raise
    def jsonToCoco(self):
        namelist, jsonlist = label_file_for_seg.json_to_coco(self.output_dir)
        if namelist and jsonlist:
            label_file_for_seg.make_coco(self.output_dir, namelist, jsonlist)
    ...
    def importDirImages(self, dirpath, pattern=None, load=True):
        ...
        for filename in self.scanAllImages(dirpath):
            ...
            if QtCore.QFile.exists(label_file) and \
                    LabelFile.is_label_file(label_file):
                item.setCheckState(Qt.Checked)
                self.checkAlienState(label_file)
            else:
                item.setCheckState(Qt.Unchecked)
    ...
    def showPopup(self, string=None, type=None):
        from PyQt5.QtWidgets import QMessageBox
        string = ""
        for i in range(len(self.tempName)):
            string += str(i+1) + ". " + self.tempName[i] + "\n"
        msg = QMessageBox()
        msg.setText("Incorrect imagepath files has been revised.\n\n" + string)
        msg.setWindowTitle('Success')
        msg.setStandardButtons(QMessageBox.Ok)
        msg.exec_()
        return

    def checkAlienState(self, path):
        try:
            import json
            with open(path, 'rb' if PY2 else 'r') as f:
                data = json.load(f)
            if data['imageData'] is not None:
                mypath = osp.abspath(osp.join(osp.dirname(path), '..'))
                mypath = osp.join(mypath, osp.basename(data['imagePath']))
                if osp.abspath(mypath) != osp.abspath(data['imagePath']):
                    self.tempName.append(osp.basename(data['imagePath']))
                    changeAlien.changePath(path, mypath)
