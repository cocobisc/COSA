    ...
    def confirm(self):
        if self.customed_radio.isChecked() == True and self.default_check.isChecked() == False \
            ...
            if float(self.learningRate_line.text()) <= 0.0:
                ...
                if self.imageGenerator_check.isChecked():
                    self.trainingTest.tableWidget.item(9, 0).setBackground(Qt.yellow)
                    self.trainingTest.tableWidget.item(9, 1).setBackground(Qt.yellow)
...
class PredictWindow(QDialog):
    ...
    def initUI(self):
        ...
        self.undo_button = QAction(QIcon('UI_image/undo_button.png'), 'Undo', self)
        self.undo_button.setShortcut('Ctrl+Z')
        self.undo_button.setStatusTip('Undo application')
        self.undo_button.triggered.connect(self.Undo_Click)
        ...
        # mainwindow.setFixedSize(100,300)
        toolbar = QToolBar()
        ...
        # self.hbox4.addWidget(self.dataset_button)
        # self.hbox4.addWidget(self.dataset_label)
    ...
    def linktest(self):
        threads = []
        if self.img_flag and self.model_path_flag:
        ...
        elif not self.img_flag :
            self.showMessageBox('Please select image')
        elif not self.model_path_flag:
    ...
    def Predict_Click(self, index):
            ...
            img = image.load_img(test_path, target_size=(model.input_shape[1], model.input_shape[2]))
            ...
            print("Label : " + self.classes[np.argmax(preds, axis=1)[0]])
